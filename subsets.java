...
public class subsets {
  ...
  public static void main(String[] args) {
    ...
    subset(0, 0, sum);
    if (c == 0) System.out.println("Subset is not possible!");
  }
  ...
  static void subset(int wsf, int k, int trw) {
    ...
    if (wsf + w[k] == d) {
      ...
      for (i = 0; i <= k; i++) {
        if (x[i] == 1) System.out.println(w[i]);
      }
      return;
    }
    if (wsf + w[k] + w[k + 1] <= d) subset(wsf + w[k], k + 1, trw - w[k]);
